  ...
  do
    case $option in
        p)
			...
			;;
        b)
            ./dependencies/thirdparty/jre-1.8.0_73-linux64b/bin/java -Xmx1024M -jar -Djava.awt.headless=true -Dspis.timestamp=$NOW -Dlogback.configurationFile=./resources/logging/logback-headless.xml -Dfelix.config.properties=file:./resources/felix/config.properties -Dactivate3DViews=true -Dorg.keridwen.config=./resources/org-spis-geo-linux64b.properties -Dorg.keridwen.headless=true -Dorg.spis.batch.script.path=$PATH_CORRECTED ./dependencies/thirdparty/felix-5.0.1/felix.jar
            exit 0
			;;
		h)
			printf "Usage:\n"
